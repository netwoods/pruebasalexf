...
//add_action('wp_ajax_dynamic_styles', 'nirvana_dynamic_styles_callback');
function nirvana_dynamic_styles_callback(){
	global $nirvanas;
    echo "html .mceContentBody, html .mceContentBody img {max-width:".$nirvanas['nirvana_sidewidth']."px;}";
	die();
}

function cryout_fetch_feed() {
	$nirvana_news = fetch_feed( array( 'http://www.cryoutcreations.eu/cat/nirvana/feed/') );
	$maxitems = 0;
